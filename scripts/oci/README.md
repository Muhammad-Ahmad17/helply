...
```bash
cp scripts/oci/env.example scripts/oci/env.local
nano scripts/oci/env.local
```
...
| Script | What it does |
|--------|--------------|
| `bash scripts/oci/provision-all.sh` | Full provision: VCN + policies + both VMs |
| `bash scripts/oci/apply-policies.sh` | Security lists/subnets only (reuse existing VMs) |
| `bash scripts/oci/show-status.sh` | Print instance state and IPs |
| `bash scripts/oci/verify-instances.sh` | SSH audit: RAM, swap, Docker, ports |

Make executable (optional):

```bash
chmod +x scripts/oci/*.sh scripts/oci/lib/*.sh
```
...
Writes `scripts/oci/.state.env` with OCIDs and IPs.
